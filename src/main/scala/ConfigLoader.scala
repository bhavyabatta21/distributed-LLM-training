...
object ConfigLoader {
  ...
  // Paths
  val localInputFilePath: String = if (config.hasPath("inputFilePath")) config.getString("inputFilePath") else "/Users/bhavyabatta/Projects/SparkLLM/src/main/resources/Input/part-r-00000"
  val s3InputPath: String = if (config.hasPath("s3InputFilePath")) config.getString("s3InputFilePath") else "s3a://default-bucket/input"
  val localModelPath: String = if (config.hasPath("localModelPath")) config.getString("localModelPath") else "/Users/bhavyabatta/Projects/SparkLLM/trained_model.zip"
  val s3ModelPath: String = if (config.hasPath("s3ModelPath")) config.getString("s3ModelPath") else "s3a://default-bucket/model.zip"
  val localLogPath: String = if (config.hasPath("localLogPath")) config.getString("localLogPath") else "/default/local/log/path"
  ...
  // Test Input
  val testInput: String = if (config.hasPath("testInput")) config.getString("testInput") else "/Users/bhavyabatta/Projects/SparkLLM/src/test/scala/resources/Input/input.txt"
}
