...
//      .master(ConfigLoader.sparkS3Master)
//      .config("spark.hadoop.fs.s3a.access.key", ConfigLoader.s3AccessKey)
//      .config("spark.hadoop.fs.s3a.secret.key", ConfigLoader.s3SecretKey)
//      .config("spark.hadoop.fs.s3a.endpoint", ConfigLoader.s3Endpoint)
//      .config("spark.hadoop.fs.defaultFS", "s3a://")
//      .getOrCreate()
    val spark = SparkSession.builder()
      .appName(ConfigLoader.sparkAppName)
    ...
    try {
      val inputFilePath = ConfigLoader.s3InputPath
