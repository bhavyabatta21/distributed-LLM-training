...
//    hadoopConfig.set("fs.s3a.endpoint", ConfigLoader.s3Endpoint)
    ...
    val localStatsPath = new Path(ConfigLoader.s3LogPath)
    val modelOutputPath = new Path(modelPath)
...
//
//    val endTime = System.currentTimeMillis()
//    writer.write(s"Total time: ${endTime - startTime} ms\n")
//    writer.flush()
//    writer.close()
//
//    // Save model to S3
...
//    }
//
//    gradientNorms.sum / gradientNorms.length
//  }
//
//  def evaluateModel(sparkModel: SparkDl4jMultiLayer, validationData: JavaRDD[DataSet]): (Double, Double, Double, Double) = {
//    val validationDataList = validationData.collect().asScala
...
//    println(s"R-squared (Accuracy): $r2")
//
//    (mse, mae, rmse, r2)
//  }
//}